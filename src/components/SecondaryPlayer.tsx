...
const SecondaryPlayer = () => {
    ...
    const createPeerConnection = () => {
      try {
        ...
        peerConnection.ontrack = (event) => {
          ...
          console.log('Stream has tracks:', event.streams?.[0]?.getTracks().length || 0);
    ...
    const handleMessage = async (event: MessageEvent) => {
      ...
      if (event.data === 'play') {
        ...
        if (event.data.type === 'webrtc-offer') {
          ...
          console.log('Received ICE candidate from primary window:', event.data.candidate ? 'candidate data' : 'null (gathering complete)');
          
          if (peerConnectionRef.current) {
            try {
              if (event.data.candidate) {
                console.log('Adding ICE candidate to peer connection');
                await peerConnectionRef.current.addIceCandidate(new RTCIceCandidate(event.data.candidate));
              ...
              setConnectionStatus(`ICE candidate error: ${err instanceof Error ? err.message : String(err)}`);
            }
          } else {
  ...
  return (
    <div className="video-container secondary">
      ...
      <p>This window is receiving a WebRTC video stream from the main window.</p>
    </div>
  )
}
